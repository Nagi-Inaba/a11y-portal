...
同日のリリース準備では、後から追加されたトップ・詳細画面を統合しました。公開側のデザインを保持し、CMSのスタイルは`cms.css`の`.cms-surface`内に限定しています。単体テスト31件、DB・公開API・CMS HTTPテスト、lint・型検査・ビルド、Chromeでの公開操作が成功しました。トップ一覧の下書き非表示・公開反映・公開停止反映と、デスクトップ5列の維持も確認しています。
